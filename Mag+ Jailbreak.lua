...
local library = loadstring(game:HttpGet('https://raw.githubusercontent.com/hlmoore/hlmoore.github.io/master/aztupuilib.lua', true))()
local main = library:CreateWindow('MAG+ Jailbreak')
...
local grav = game:GetService("Workspace").Gravity
local e = 0
local fflyen = 0
...
local toggleESP = main:Toggle('ESP Toggle', function(state)
    if state then
        on = true
        for i,v in pairs(game.Players:GetChildren()) do
            if v.Character ~= game.Players.LocalPlayer.Character and v.Character.Head:FindFirstChild('ScreenGui') == nil then
...
local toggleFly = main:Toggle('F to Airjump', function(state)
    if state then
        fflyen = 1
    else 
        fflyen = 0
...
local bringallunanchored = main:Button('Bring All Unanchored Parts', function()
